#!/bin/sh
# End-to-end: install vernacular at local scope in a scratch repo and prove the command is
...
# (guardtower -> vernacular, .guardtower -> .vernacular). They encode hard-won behaviour that is
# not specific to either plugin: a portable timeout escalation, a classifier that separates a
# transient API failure from a real regression, and quorum retry for single-turn narration
# flakiness.
...
cleanup() {
  ...
  # add.
  [ "$plugin_state" = "absent" ] && claude plugin uninstall vernacular@dashworthy --scope local >/dev/null 2>&1
...
plugin_state=$(detect_state 'vernacular@dashworthy' claude plugin list)

if [ "$marketplace_state" != "absent" ] || [ "$plugin_state" != "absent" ]; then
  printf 'NOTE - marketplace "dashworthy" state: %s. plugin vernacular@dashworthy state: %s.\n' "$marketplace_state" "$plugin_state"
  ...
  printf '\nINCONCLUSIVE - could not safely confirm dashworthy/vernacular@dashworthy is clear; this run does not confirm a regression.\n'
  exit 2
fi
